...
4. **Evidências cruzadas:** cada afirmação de causa MUST citar pelo menos duas fontes quando possível (ex.: log + métrica, ou config + log). Quando presentes nos artefatos, cite identificadores concretos (ex.: `reindex task [88123]`, shard `[logs-2026.05][7]`, `avg_duration_min: 90`).
5. **Honestidade epistêmica:** se os dados não permitem conclusão definitiva, declare explicitamente o que **não** pode ser afirmado — inclua **pelo menos 2 limites** concretos (ex.: por que a reindexação estagnou, visibilidade parcial do cluster, ausência de dados após o fim da janela).
6. **Ação proporcional:** recomende ação imediata coerente (ex.: pausar/cancelar reindexação identificada nos logs) e follow-up específico (investigar atraso da reindexação vs. média histórica; revisar `jvm_heap`, `refresh_interval` ou limites do shard quente) — sem playbook genérico.
...
O que os artefatos **não** permitem concluir com segurança (liste **≥ 2** itens em bullets):
